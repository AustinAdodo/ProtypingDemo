// npm install chai. npm install --save-dev mocha   npx mocha ./ProtypingDemo/Human.test.js
config.truncateThreshold = 0;
const { expect, config, assert } = require("chai");
const package = require("./Human.js");
const head = package.head;
const eye = package.eye;
const Thing = package.Thing;
const finger = package.finger;
const hand = package.hand;
...
describe("Thing", () => {
  ...
  describe("#has", () => {
    ...
    describe("jane.has(1).head.having(2).eyes", () => {
      ...
      it("should create 2 new things on the head", () => {
        ...
        expect(jane.head.eyes.every((eye) => eye instanceof Thing)).to.equal(
          true
        );
      });
  ...
  describe("#each", () => {
    describe("jane.has(2).hands.each(hand => having(5).fingers)", () => {
      ...
      jane.has(2).hands.each(() => having(5).fingers);
      let c = [];
